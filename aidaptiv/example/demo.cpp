...
int main(int argc, char ** argv) {
    ...
    model_params.vram_experts_cached_gb = static_cast<uint32_t>(vram_experts_cached_gb);
    model_params.dram_experts_cached_gb = static_cast<uint32_t>(dram_experts_cached_gb);
    ...
    auto restore_stats = adptv.restore_kv_cache(prompt_tokens, {}, lora_adapters, 0, 0);
    ...
    // Skip prompt tokens whose KV cache was just restored by aidaptiv. Always
    // keep at least one token in the batch so the sampler has fresh logits.
