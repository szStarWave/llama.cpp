...
```cpp
auto stats = adptv.restore_kv_cache(prompt_tokens, {}, {}, /*slot_id*/ 0, /*hit_in_device*/ 0);
...
// Skip prompt tokens whose KV was just restored. Always keep at least one
// token in the batch so the sampler has fresh logits.
uint32_t skip = total_reuse;
if (skip >= (uint32_t) prompt_tokens.size()) {
    skip = (uint32_t) prompt_tokens.size() - 1;
}
...
| If `skip >= prompt_tokens.size()`, use `skip = prompt_tokens.size() - 1` | You must leave at least one prompt token in the batch so the sampler gets valid logits. |
...
4. Pass the **same** `mtmd_info` to `restore_kv_cache()` and `save_kv_cache()`.
...
auto stats = adptv.restore_kv_cache(prompt_tokens, mtmd_info, lora_adapters, slot_id, hit_in_device);
// ... decode ...
...
auto stats = adptv.restore_kv_cache(prompt_tokens, {}, lora_adapters, slot_id, hit_in_device);
// ... decode ...
...
- Copy next to **demo.exe**: `ada.exe`, `wService_create.bat`, and `wService_delete.bat`
...
CMD
```cmd
demo.exe -m <model> [-n n_predict] [-o offload_path]
...
```cmd
demo.exe -m gpt-oss-20b-Q4_K_M.gguf -n 128 -o D:\\ -vc 3 -sk -1 -dk -1
```
...
demo.exe -m base.gguf -o D:\\ -lora adapter.gguf -ls 1.0 -sk -1 -dk -1 -kr 1 -n 16
...
// Free space: dynamic caches are evicted
