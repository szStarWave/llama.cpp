...
static void print_usage(int /* argc */, char ** argv) {
    ...
    printf("  -rq <auto|on|off>                           load-time requant of always-on tensors to Q4_K. (default: auto = on for qwen35/qwen35moe/gemma4/qwen3)\n");
    printf("\n");
    printf(
        ...
        try {
            if (arg == "-h" || arg == "--help") {
                ...
                if (v == "on" || v == "1" || v == "true") {
                    ...
                    fprintf(stderr, "error: invalid -rq value: '%s' (expected auto|on|off)\n", v.c_str());
                    invalid_param = true;
                    break;
                }
...
int main(int argc, char ** argv) {
    ...
    for (const auto & inst : params_instances) {
        ...
        if (!params.n_ubatch_explicit) {
            ...
            char expert_buf[32] = {};
            bool is_moe = false;
            if (llama_model_meta_val_str(lmodel, "general.architecture", arch_buf, sizeof(arch_buf)) >= 0) {
                std::string expert_key = std::string(arch_buf) + ".expert_count";
                is_moe = llama_model_meta_val_str(lmodel, expert_key.c_str(), expert_buf, sizeof(expert_buf)) >= 0
                         && std::atoi(expert_buf) > 1;
            }
            cparams.n_ubatch = (is_moe && inst.flash_attn) ? 2048 : cmd_params_defaults.n_ubatch[0];
        }
