...
void llama_model_openai_moe::load_arch_tensors(llama_model_loader &) {
    ...
    const expert_tensor_params expert_params = {
        ...
        { LLM_TENSOR_FFN_GATE_EXPS, "bias",   { n_ff_exp },         false },
        { LLM_TENSOR_FFN_DOWN_EXPS, "bias",   { n_embd },           false },
        { LLM_TENSOR_FFN_UP_EXPS,   "bias",   { n_ff_exp },         false }
    };
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (is_moe_offload_enabled() && !need_exclude(i)) {
            layer.ffn_gate_exps_b = expert_mapping_table[tn(LLM_TENSOR_FFN_GATE_EXPS, "bias", i).str()];
            layer.ffn_down_exps_b = expert_mapping_table[tn(LLM_TENSOR_FFN_DOWN_EXPS, "bias", i).str()];
            layer.ffn_up_exps_b   = expert_mapping_table[tn(LLM_TENSOR_FFN_UP_EXPS, "bias", i).str()];
        } else {
