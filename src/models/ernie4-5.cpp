...
void llama_model_ernie4_5::load_arch_tensors(llama_model_loader &) {
    ...
    const auto is_moe_layer = [&](uint32_t i) -> bool {
        return arch == LLM_ARCH_ERNIE4_5_MOE && i >= hparams.n_layer_dense_lead;
    };

    expert_tensor_params expert_params = {};
    std::unordered_map<std::string, ggml_tensor *> expert_mapping_table;
    ...
    if (arch == LLM_ARCH_ERNIE4_5_MOE) {
        ...
        expert_mapping_table = is_moe_offload_enabled() ? distribute_expert_tensor(expert_params, n_layer, is_moe_layer) :
    ...
    if (arch == LLM_ARCH_ERNIE4_5_MOE && is_moe_offload_enabled()) {
