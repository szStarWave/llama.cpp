...
struct vk_matmul_pipeline_struct {
    ...
    // ggml_vk_guess_matmul_pipeline when n<128 && m<2048. Null for non-Intel
...
struct vk_device_struct {
    ...
    vk_pipeline pipeline_flash_attn_gqa4_hdim64;
    vk_pipeline pipeline_flash_attn_mha_hdim96;
    vk_pipeline pipeline_flash_attn_hdim128;
    vk_pipeline pipeline_flash_attn_gqa2_hdim128;
    ...
    vk_pipeline pipeline_flash_attn_decode_hdim128_gqa2_ph1;
    vk_pipeline pipeline_flash_attn_decode_hdim128_gqa2_ph2;
    vk_pipeline pipeline_flash_attn_decode_hdim128_gqa4_ph1;
    vk_pipeline pipeline_flash_attn_decode_hdim128_gqa4_ph2;
    vk_pipeline pipeline_flash_attn_decode_hdim128_gqa6_ph1;
    vk_pipeline pipeline_flash_attn_decode_hdim128_gqa6_ph2;
    ...
    vk_pipeline pipeline_flash_attn_decode_hdim64_gqa4_ph1;
    vk_pipeline pipeline_flash_attn_decode_hdim64_gqa4_ph2;
    ...
    vk_pipeline pipeline_flash_attn_decode_hdim256_gqa6_ph1;
    vk_pipeline pipeline_flash_attn_decode_hdim256_gqa6_ph2;
    vk_pipeline pipeline_flash_attn_decode_hdim512_gqa8_ph1;
    vk_pipeline pipeline_flash_attn_decode_hdim512_gqa8_ph2;
    vk_pipeline pipeline_flash_attn_decode_hdim96_mha_ph1;
    vk_pipeline pipeline_flash_attn_decode_hdim96_mha_ph2;
...
struct vk_flash_attn_hdim64_opt_push_constants {
    ...
    uint32_t kvHead;
    uint32_t qkRatio;
    uint32_t qkSubGroups;
    uint32_t flag;
    uint32_t batchStrideQ;
    uint32_t batchStrideK;
    uint32_t batchStrideV;
    uint32_t batchStrideM;
    uint32_t batchStrideO;
    float softMaxScale;
};
...
static vk_fa_tuning_params get_fa_tuning_params(const vk_device& device, uint32_t hsk, uint32_t hsv, uint32_t n_rows, uint32_t n_kv, ggml_type k_type, ggml_type v_type, bool f32acc) {
    ...
    // TURBO K/V is implemented by scalar/coopmat1 FA decode; coopmat2 has a separate decode path.
    if (k_type == GGML_TYPE_TURBO3_0 || k_type == GGML_TYPE_TURBO4_0 ||
...
#undef CREATE_MM2
    ...
    if (device->coopmat_support) {
        ...
        std::vector<uint32_t>   _saved_wt;
        std::array<uint32_t, 3> _saved_wgd{};
        uint32_t                _saved_al = 0;

        auto set_warp_tile = [&](std::vector<uint32_t>   wt,
                                  std::array<uint32_t, 3> wgd,
                                  uint32_t al) {
            _saved_wt  = l_warptile_mmq;
            _saved_wgd = l_mmq_wg_denoms;
            _saved_al  = l_align;
        ...
        auto restore_warp_tile = [&]() {
            l_warptile_mmq  = std::move(_saved_wt);
            l_mmq_wg_denoms = _saved_wgd;
            l_align         = _saved_al;
        };
...
#define CREATE_MM2(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID) \
        ...
        // Intel-only: build the alt l/a_l pipelines into the ->l_alt / ->a_l_alt slots.
...
#define CREATE_MM2_ALT(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID) \
        ...
        // Intel-only: build the ALT l/a_l pipelines into ->l_alt / ->a_l_alt for f16-B-type
...
#undef CREATE_MM
    ...
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_TURBO3_0], "dequant_turbo3_0", dequant_turbo3_0_len, dequant_turbo3_0_data, "main", 2, 5 * sizeof(uint32_t), {256 * 32, 1, 1}, {}, 1);
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_TURBO4_0], "dequant_turbo4_0", dequant_turbo4_0_len, dequant_turbo4_0_data, "main", 2, 5 * sizeof(uint32_t), {256 * 32, 1, 1}, {}, 1);
    ...
    if (device->architecture == INTEL_XE2) {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_gqa4_hdim64, "pipeline_flash_attn_gqa4_hdim64", fa_hdim_64_cm1_len, fa_hdim_64_cm1_data, "main", 6, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 64, 1, 1 }, { 256, 2, 32, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_mha_hdim96, "pipeline_flash_attn_mha_hdim96", fa_hdim_96_cm1_len, fa_hdim_96_cm1_data, "main", 6, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 256, 1, 1 }, { 256, 0, 16, 16 }, 1, false, true, 16);
        ...
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_gqa2_hdim128, "pipeline_flash_attn_gqa2_hdim128", fa_hdim_128_cm1_len, fa_hdim_128_cm1_data, "main", 6, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 64, 1, 1 }, { 256, 1, 16, 16 }, 1, false, true, 16);
        ...
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa2_ph1, "pipeline_flash_attn_decode_hdim128_gqa2_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 2, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa2_ph2, "pipeline_flash_attn_decode_hdim128_gqa2_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 2, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa4_ph1, "pipeline_flash_attn_decode_hdim128_gqa4_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 4, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa4_ph2, "pipeline_flash_attn_decode_hdim128_gqa4_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 4, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa6_ph1, "pipeline_flash_attn_decode_hdim128_gqa6_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 6, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa6_ph2, "pipeline_flash_attn_decode_hdim128_gqa6_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 6, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa8_ph1, "pipeline_flash_attn_decode_hdim128_gqa8_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 8, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa8_ph2, "pipeline_flash_attn_decode_hdim128_gqa8_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 8, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim64_gqa8_ph1, "pipeline_flash_attn_decode_hdim64_gqa8_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 8, 64, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim64_gqa8_ph2, "pipeline_flash_attn_decode_hdim64_gqa8_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 8, 64, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim64_gqa4_ph1, "pipeline_flash_attn_decode_hdim64_gqa4_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 4, 64, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim64_gqa4_ph2, "pipeline_flash_attn_decode_hdim64_gqa4_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 4, 64, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa8_ph1, "pipeline_flash_attn_decode_hdim256_gqa8_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 8, 256, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa8_ph2, "pipeline_flash_attn_decode_hdim256_gqa8_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 8, 256, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa2_ph1, "pipeline_flash_attn_decode_hdim256_gqa2_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 2, 256, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa2_ph2, "pipeline_flash_attn_decode_hdim256_gqa2_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 2, 256, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa6_ph1, "pipeline_flash_attn_decode_hdim256_gqa6_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 6, 256, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa6_ph2, "pipeline_flash_attn_decode_hdim256_gqa6_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 6, 256, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim512_gqa8_ph1, "pipeline_flash_attn_decode_hdim512_gqa8_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 8, 512, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim512_gqa8_ph2, "pipeline_flash_attn_decode_hdim512_gqa8_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 8, 512, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim96_mha_ph1, "pipeline_flash_attn_decode_hdim96_mha_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 1, 96, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim96_mha_ph2, "pipeline_flash_attn_decode_hdim96_mha_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 1, 96, 16 }, 1, false, true, 16);
    }
    else if (device->architecture == INTEL_XE1 && device->coopmat_support && device->uma) {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_gqa4_hdim64, "pipeline_flash_attn_gqa4_hdim64", fa_hdim_64_cm1_len, fa_hdim_64_cm1_data, "main", 6, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 64, 1, 1 }, { 256, 2, 16, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_mha_hdim96, "pipeline_flash_attn_mha_hdim96", fa_hdim_96_cm1_len, fa_hdim_96_cm1_data, "main", 6, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 256, 1, 1 }, { 256, 0, 16, 8 }, 1, false, true, 8);
        ...
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_gqa2_hdim128, "pipeline_flash_attn_gqa2_hdim128", fa_hdim_128_cm1_len, fa_hdim_128_cm1_data, "main", 6, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 64, 1, 1 }, { 256, 1, 16, 8 }, 1, false, true, 8);
        ...
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa2_ph1, "pipeline_flash_attn_decode_hdim128_gqa2_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 2, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa2_ph2, "pipeline_flash_attn_decode_hdim128_gqa2_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 2, 128, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa4_ph1, "pipeline_flash_attn_decode_hdim128_gqa4_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 4, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa4_ph2, "pipeline_flash_attn_decode_hdim128_gqa4_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 4, 128, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa6_ph1, "pipeline_flash_attn_decode_hdim128_gqa6_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 6, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa6_ph2, "pipeline_flash_attn_decode_hdim128_gqa6_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 6, 128, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa8_ph1, "pipeline_flash_attn_decode_hdim128_gqa8_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 8, 128, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim128_gqa8_ph2, "pipeline_flash_attn_decode_hdim128_gqa8_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 8, 128, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim64_gqa4_ph1, "pipeline_flash_attn_decode_hdim64_gqa4_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 4, 64, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim64_gqa4_ph2, "pipeline_flash_attn_decode_hdim64_gqa4_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 4, 64, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim64_gqa8_ph1, "pipeline_flash_attn_decode_hdim64_gqa8_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 8, 64, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim64_gqa8_ph2, "pipeline_flash_attn_decode_hdim64_gqa8_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 8, 64, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa8_ph1, "pipeline_flash_attn_decode_hdim256_gqa8_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 8, 256, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa8_ph2, "pipeline_flash_attn_decode_hdim256_gqa8_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 8, 256, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa2_ph1, "pipeline_flash_attn_decode_hdim256_gqa2_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 2, 256, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa2_ph2, "pipeline_flash_attn_decode_hdim256_gqa2_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 2, 256, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa6_ph1, "pipeline_flash_attn_decode_hdim256_gqa6_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 6, 256, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim256_gqa6_ph2, "pipeline_flash_attn_decode_hdim256_gqa6_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 6, 256, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim512_gqa8_ph1, "pipeline_flash_attn_decode_hdim512_gqa8_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 8, 512, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim512_gqa8_ph2, "pipeline_flash_attn_decode_hdim512_gqa8_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 8, 512, 8 }, 1, false, true, 8);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim96_mha_ph1, "pipeline_flash_attn_decode_hdim96_mha_ph1", fa_decode_ph1_cm1_len, fa_decode_ph1_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 1, 64, 1 }, { 128, 1, 96, 16 }, 1, false, true, 16);
        ggml_vk_create_pipeline(device, device->pipeline_flash_attn_decode_hdim96_mha_ph2, "pipeline_flash_attn_decode_hdim96_mha_ph2", fa_decode_ph2_cm1_len, fa_decode_ph2_cm1_data, "main", 5, sizeof(vk_flash_attn_hdim64_opt_push_constants), { 16, 1, 1 }, { 128, 1, 96, 8 }, 1, false, true, 8);
    }
    ...
    ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_TURBO4_0], "cpy_f32_turbo4_0", cpy_f32_turbo4_0_len, cpy_f32_turbo4_0_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
...
static void ggml_vk_mul_mat_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool disable_split_k) {
    ...
    const bool y_non_contig = (ctx->device->coopmat2 && src1->type == GGML_TYPE_F32) ||
                              (ctx->device->coopmat_support && !ctx->device->coopmat2 && ggml_is_quantized(src0->type) && src1->type == GGML_TYPE_F32) ||
...
static void ggml_vk_flash_attn(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * q, const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask, const ggml_tensor * sinks, ggml_tensor * dst) {
    ...
    uint32_t activationPerSplit = 512;
    {
        ...
        if (xe_fa_supported_platform && q->nb[1] > q->nb[2]) {
            if (neq0 == 64 && (qk_ratio == 8 || qk_ratio == 4) && ctx->device->pipeline_flash_attn_hdim64 != nullptr) {
                ...
                case 4:
                    if (neq1 == 1) {
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim64_gqa4_ph1);
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim64_gqa4_ph2);
                        fa_copy_qstate = false;
                    }
                    else {
                        xe_fa_pipeline = ctx->device->pipeline_flash_attn_gqa4_hdim64;
                        fa_copy_qstate = false;
                    }
                    break;
                }
            }
            if (neq0 == 96 && (qk_ratio == 0) && ctx->device->pipeline_flash_attn_mha_hdim96 != nullptr) {
                xe_fa_opt = true;
                fa_copy_qstate = false;
                switch (qk_ratio) {
                case 1:
                default:
                    if (neq1 == 1) {
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim96_mha_ph1);
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim96_mha_ph2);
                        fa_copy_qstate = false;
                    }
                    else {
                        xe_fa_pipeline = ctx->device->pipeline_flash_attn_mha_hdim96;
                        fa_copy_qstate = false;
                    }
                    break;
                }
            }
            else if (neq0 == 128 && (qk_ratio == 4 || qk_ratio == 8 || qk_ratio == 2 || qk_ratio == 6) && ctx->device->pipeline_flash_attn_hdim128 != nullptr) {
                xe_fa_opt = true;
                switch (qk_ratio) {
                case 4:
                default:
                ...
                case 2:
                    if (neq1 == 1) {
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim128_gqa2_ph1);
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim128_gqa2_ph2);
                        fa_copy_qstate = false;
                    }
                    else {
                        xe_fa_pipeline = ctx->device->pipeline_flash_attn_gqa2_hdim128;
                        fa_copy_qstate = false;
                    }
                    break;
                case 6:
                    if (neq1 == 1) {
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim128_gqa6_ph1);
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim128_gqa6_ph2);
                        fa_copy_qstate = false;
                    }
                    else {
                        xe_fa_pipeline = ctx->device->pipeline_flash_attn_gqa2_hdim128;
            ...
            else if (neq0 == 256 && ((qk_ratio == 8) || (qk_ratio == 2) /* || (qk_ratio == 6) */) && ctx->device->pipeline_flash_attn_gqa2_hdim256_ph1 != nullptr) {
                xe_fa_opt = true;
                switch (qk_ratio) {
                case 2:
                default:
                ...
                case 6:
                    if (neq1 == 1) {
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim256_gqa6_ph1);
                        xe_fa_decode_pipeline.emplace_back(ctx->device->pipeline_flash_attn_decode_hdim256_gqa6_ph2);
                        fa_copy_qstate = false;
                        xe_fa_opt = true;
                    }
                    else {
    ...
    if (xe_fa_opt) {
        ...
        size_t tmp_size = (uint64_t)(32768 * 17 / 16 * neq2 * batches * sizeof(float));
        if (neq1 == 1) { // decode temp buffer
            tmp_size = std::max(tmp_size, (uint64_t)((nek1 + 4095) / 4096 * 4096) * 17 / 16 * neq2 * batches * sizeof(float));
        }

        if (xe_fa_prefill_pipeline.size() == 2 && neq1 > 1) {
            size_t prefillTempSz = sizeP + sizeGroupMax + sizeGlobalMax;
            if (fa_copy_qstate == true) {
                prefillTempSz = prefillTempSz + x_ne * sizeof(ggml_fp16_t);
            }
        ...
        if (xe_fa_decode_pipeline.size() == 2 && neq1 == 1) { // 2 phase decode
            size_t sizeDecodeP = batches * neq2 * nek1 * sizeof(float);
            vk_subbuffer p_temp_buf = ggml_vk_subbuffer(ctx, ctx->prealloc_fattn_temp, 0);
            vk_subbuffer max_temp_buf = ggml_vk_subbuffer(ctx, ctx->prealloc_fattn_temp, sizeDecodeP);
            vk_flash_attn_hdim64_opt_push_constants fattnConsts;
            uint32_t qLen = neq1;
            int32_t gqaRatio = (int32_t)(1 << power_of_2(qk_ratio));
            int32_t qkGroups = qk_ratio / gqaRatio;
            ...
            fattnConsts.qkRatio = qk_ratio;
            fattnConsts.qkSubGroups = qkGroups;
            fattnConsts.batchStrideQ = nbq3 / ggml_type_size(q->type);
            fattnConsts.batchStrideK = nbk3 / ggml_type_size(k->type);
            fattnConsts.batchStrideV = nbv3 / ggml_type_size(v->type);
            fattnConsts.batchStrideM = mask->nb[3] / ggml_type_size(mask->type);
            fattnConsts.batchStrideO = nb3 / ggml_type_size(dst->type);
            ggml_vk_sync_buffers(ctx, subctx);
            ...
            ggml_vk_dispatch_pipeline(ctx, subctx, xe_fa_decode_pipeline.at(0),
                ...
                fattnConsts, { (uint32_t)nek2, (uint32_t)nek1, (uint32_t)batches });
            ggml_vk_sync_buffers(ctx, subctx);
            ...
            ggml_vk_dispatch_pipeline(ctx, subctx, xe_fa_decode_pipeline.at(1),
                ...
                fattnConsts, { (uint32_t)neq0, (uint32_t)nek2, (uint32_t)batches });
        }
            ...
            int32_t pLoop = qk_ratio / gqaRatio;
            ...
            for (int64_t bl = 0; bl < batches; bl++) {
                for (int32_t ql = 0; ql < qLoop; ql++) {
                    ...
                    for (int32_t pl = 0; pl < pLoop; pl++) {
                        fattnConsts.startHeadQ = pl * gqaRatio;
            ...
            ggml_vk_dispatch_pipeline(ctx, subctx, xe_fa_pipeline,
                ...
                fattnConsts, { (uint32_t)qLen, (uint32_t)(qkGroups * nek2), (uint32_t)batches });
        }
    }
    else {
    if (split_k > 1) {
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            {
                ...
                const bool uses_scalar_or_cm1_fa =
                    !coopmat2 ||
                    op->src[1]->type == GGML_TYPE_TURBO3_0 || op->src[1]->type == GGML_TYPE_TURBO4_0 ||
                    op->src[2]->type == GGML_TYPE_TURBO3_0 || op->src[2]->type == GGML_TYPE_TURBO4_0;
                if (uses_scalar_or_cm1_fa && !(device->subgroup_shuffle && device->subgroup_vote)) {
...
static uint32_t ggml_vk_intel_shader_core_count(const vk::PhysicalDevice& vkdev) {
    ...
    case 0xB080:  // PTL Xe3 LPG 2x6 (12 subslices)
        return 12;
    case 0xFD80:  // WCL XE3 LPG 2x1 (2 subslices)
    case 0xFD81:
        return 2;
    default:
        return 0;
